...
function openEditor(spec, caret) {
  ...
  ed.addEventListener('compositionend', () => {
    ...
    if (state.editing.locked) {
      setTimeout(() => {
        if (state.editing && state.editing.locked && !state.editing.composing) {
          schedulePreview();
        }
      }, 10);
    }
...
function isKnownCjkFamily(fam) {
  if (!fam) return false;
  return /noto sans (sc|tc|cjk)|microsoft yahei|simsun|simhei|kaiti|fangsong|pingfang|hiragino|songti|stheiti|meiryo|gothic/i.test(
    fam
  );
}

function splitMixedScriptRuns(text, st) {
  if (!text) return [];
  if (text === '\n' || text === '\uFFFC') return [{ ...st, text }];
  if (isKnownCjkFamily(st.family)) return [{ ...st, text }];
  let hasCjk = false;
  ...
  if (!hasCjk) return [{ ...st, text }];
  const cjkFamily = 'Noto Sans SC';
  const out = [];
  let curIsCjk = isCjkChar(text.codePointAt(0));
  let curText = '';
  for (let i = 0; i < text.length; i++) {
    const cp = text.codePointAt(i);
    const cjk = isCjkChar(cp);
    if (cjk === curIsCjk) {
      curText += text[i];
    } else {
      if (curText) {
        out.push({
          ...st,
          family: curIsCjk ? cjkFamily : st.family,
          text: curText,
        });
      }
      curIsCjk = cjk;
...
function parseEditor(ed, original) {
  ...
  const pushRun = (text, span) => {
    ...
    const subRuns = splitMixedScriptRuns(text, st);
