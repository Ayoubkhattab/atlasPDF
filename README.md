...
<div align="center">
  <img src="public/images/logo.png" alt="AtlasPDF Logo" width="120" height="120" />
...
[![Website](https://img.shields.io/website?url=https%3A%2F%2Fatlaspdf.mis-pts.org%2Fen%2F)](https://atlaspdf.mis-pts.org/en/)
...
**AtlasPDF** is a comprehensive suite of PDF tools designed for privacy and performance. Unlike many online converters, AtlasPDF processes your files entirely within your browser using WebAssembly technology. Your documents **never** leave your device, ensuring maximum security for your sensitive data.
...
AtlasPDF includes a powerful **visual workflow editor** that allows you to chain multiple PDF operations together, creating automated processing pipelines.
...
1.  **Clone the repository**
    ```bash
    git clone https://github.com/Ayoubkhattab/atlasPDF.git
    cd atlaspdf
    ```
...
AtlasPDF provides both pre-built Docker images and Docker Compose for flexible deployment options.

#### Option 1: Use Pre-built Image (Recommended)

The easiest way to run AtlasPDF is using our pre-built image from GitHub Container Registry:

```bash
# Pull the latest image
docker pull ghcr.io/atlaspdftool/atlaspdf:latest

# Run the container
docker run -d -p 8080:80 --name atlaspdf ghcr.io/atlaspdftool/atlaspdf:latest
```

Open [http://localhost:8080](http://localhost:8080) to access AtlasPDF.
...
AtlasPDF supports deployment under a subpath (e.g., `https://your-domain.com/atlaspdf/`). To enable this, you must specify the base path during the build process.
...
AtlasPDF is configured for static export (`output: 'export'`), which means it can be deployed to any service that supports static website hosting without requiring a Node.js server.
...
AtlasPDF stands on the shoulders of giants. We gratefully acknowledge [BentoPDF](https://github.com/alam00000/bentopdf) for their pioneering work in privacy-first, client-side PDF tools.

Their project served as a significant inspiration and reference for our core logic. While AtlasPDF has been re-engineered for the Next.js ecosystem and extends functionality with unique features like the *Workflow Editor*, we deeply respect the foundation laid by the BentoPDF team.
...
<div align="center">
  Built with ❤️ by the AtlasPDF Team
</div>
