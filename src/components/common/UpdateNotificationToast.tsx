...
export const UpdateNotificationToast: React.FC<UpdateNotificationToastProps> = ({
  ...
  return (
    <aside
      ...
      className="fixed bottom-5 right-5 z-[9999] w-[calc(100vw-2.5rem)] sm:w-[380px] rounded-2xl border border-[color-mix(in_srgb,var(--color-primary)_30%,transparent)] bg-[var(--color-background)] shadow-2xl p-4 transition-all animate-in slide-in-from-bottom-5 fade-in duration-300 ring-1 ring-[color-mix(in_srgb,var(--color-primary)_20%,transparent)]"
    >
      {/* Header bar */}
      <div className="flex items-start justify-between gap-3 mb-2.5">
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-[var(--color-primary)] text-white shadow-sm">
      ...
      <p className="text-xs text-[var(--color-muted-foreground)] mb-3.5 leading-relaxed">
        {getMsg(
          'toastPrompt',
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between text-[11px] pt-1 text-[var(--color-muted-foreground)]">
          <button
            type="button"
            onClick={onSnooze}
            className="inline-flex items-center gap-1 py-0.5 px-1.5 rounded hover:text-[var(--color-foreground)] hover:bg-[var(--color-muted)] transition-colors"
          >
