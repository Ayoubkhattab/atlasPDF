...
export function MOBIToPDFTool({ className = '' }: MOBIToPDFToolProps) {
    ...
    const handleConvert = useCallback(async () => {
        if (!file) {
            setError(t('errors.uploadFile') || 'Please upload a MOBI file.');
            return;
        }
        ...
        try {
            const output: ProcessOutput = await mobiToPDF(
                file.file,
                {},
                (prog, message) => {
            ...
            if (output.success && output.result) {
                ...
                setError(output.error?.message || 'Failed to convert MOBI to PDF.');
                setStatus('error');
            }
        } catch (err) {
            if (!cancelledRef.current) {
    ...
    return (
        <div className={`space-y-8 ${className}`.trim()}>
            <FileUploader
                accept={['.mobi', '.azw', '.azw3', 'application/x-mobipocket-ebook']}
                multiple={false}
                maxFiles={1}
                onFilesSelected={handleFilesSelected}
                onError={handleUploadError}
                disabled={isProcessing}
                label={tTools('mobiToPdf.uploadLabel') || 'Upload MOBI File'}
                description={tTools('mobiToPdf.uploadDescription') || 'Drag and drop a MOBI e-book file here, or click to browse.'}
            />
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
                <Button variant="primary" size="lg" onClick={handleConvert} disabled={!canConvert} loading={isProcessing} className="min-w-[200px] shadow-lg hover:shadow-primary/25 transition-all hover:-translate-y-0.5">
                    ...
                    {isProcessing ? (t('status.processing') || 'Converting...') : (tTools('mobiToPdf.convertButton') || 'Convert to PDF')}
                </Button>

                {result && !Array.isArray(result) && (
                    <DownloadButton file={result as Blob} filename={`${file?.file.name.replace(/\.(mobi|azw|azw3)$/i, '')}.pdf`} variant="secondary" size="lg" showFileSize className="min-w-[200px] shadow-lg transition-all hover:-translate-y-0.5" />
                )}
            </div>
            ...
            {status === 'complete' && result && (
                <div className="p-6 rounded-2xl bg-green-50/50 border border-green-200 text-green-700 text-center animate-in fade-in zoom-in-95 duration-300" role="status">
                    ...
                    <p className="text-green-800/80 max-w-md mx-auto">
                        {tTools('mobiToPdf.successMessage') || 'Your MOBI file has been converted to PDF.'}
                    </p>
                </div>
            )}
        </div>
