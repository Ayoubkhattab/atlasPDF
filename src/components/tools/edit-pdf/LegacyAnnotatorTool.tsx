...
export interface LegacyAnnotatorToolProps {
  className?: string;
  file?: File | null;
 ...
 */
export function LegacyAnnotatorTool({
  className = '',
  file: externalFile,
  onFileChange,
  hideHeaderCard = false,
  onClear,
}: LegacyAnnotatorToolProps) {
  ...
  // Sync external file prop changes
  useEffect(() => {
    if (externalFile) {
      setError(null);
...
export default LegacyAnnotatorTool;
