...
export class RTFToPDFProcessor extends BasePDFProcessor {
    ...
    async process(
        ...
        // RTF conversion has no fallback engine - it always needs LibreOffice WASM,
        // which needs SharedArrayBuffer. Fail fast with an actionable message instead
        // of spending a ~250MB download attempt on a doomed conversion.
        if (!isCrossOriginIsolated()) {
            return this.createErrorOutput(
                PDFErrorCode.PROCESSING_FAILED,
                'RTF conversion requires LibreOffice, which needs Cross-Origin Isolation on your server.',
